...
describe("buildCues", () => {
  ...
  it("режет по концу предложения даже без паузы", () => {
    // Гэп после "Читай." (1.02-1.0=0.02с) специально мал, но не сливается:
    // у ПЕРВОЙ группы собственная длительность уже ≥ MIN_DUR_SEC, поэтому
    // её растягивать не нужно и её конец не заезжает на начало второй —
    // растяжение второй группы (её собственная длительность всего 0.48с,
    // короче MIN_DUR_SEC) двигает только её СОБСТВЕННЫЙ конец вперёд и не
    // может создать перекрытие с предыдущим блоком. Тест проверяет именно
  ...
  it("не сливает блоки, если слияние пробило бы MAX_DUR_SEC — режет зазор до нуля (Ruling 13)", () => {
    // "Ya." растягивается до 0.9с и залезает на "Panjang..." (старт 0.35с).
    // Слить их означало бы блок 2.65с > MAX_DUR_SEC(2.6) — приоритет 2
    // сильнее приоритета 3, поэтому вместо слияния подрезаем конец первого
    // блока к началу второго (зазор до нуля) и принимаем первый блок короче
    // MIN_DUR_SEC — это осознанная уступка, а не баг Ruling 10.
    const cues = buildCues([w("Ya.", 0, 0.3), w("Panjang sekali dan berkepanjangan", 0.35, 2.65)]);
    expect(cues).toHaveLength(2);
    expect(cues[0].end).toBe(cues[1].start); // зазор ровно ноль, перекрытия нет
    expect(cues[0].end - cues[0].start).toBeLessThan(MIN_DUR_SEC); // законно короче нижней границы
    expect(cues[1].end - cues[1].start).toBeLessThanOrEqual(MAX_DUR_SEC);
  });

  it("splitLongGroup режет равнотемповую группу близко к середине, а не отгрызанием слова с фронта (Ruling 13)", () => {
    // 10 слов с ОДИНАКОВЫМИ паузами 0.05с — фикстура из брифа. Со строгим
    // `>` тай-брейк раньше оставлял позицию 0 (первое слово отдельно, а
    // остаток рекурсивно отгрызался тем же способом дальше); с допуском и
    // предпочтением середины группа делится пополам: 5 слов / 5 слов.
    const words: Word[] = Array.from({ length: 10 }, (_, i) => w(`word${i}`, i * 0.5, i * 0.5 + 0.45));
    const cues = buildCues(words);
    expect(cues).toHaveLength(2);
    expect(cues[0].ru.split(" ")).toHaveLength(5);
    expect(cues[1].ru.split(" ")).toHaveLength(5);
    expect(cues[0].ru).toBe("word0 word1 word2 word3 word4");
    expect(cues[1].ru).toBe("word5 word6 word7 word8 word9");
  });

  it.each([10, 20, 40])(
    "ни один блок не длиннее MAX_DUR_SEC на равнотемповой фикстуре из %i слов (Ruling 13, регресс на выводе ревьюера)",
    (n) => {
      // До правки: 10 слов → 3.95с, 20 слов → 7.4с, 40 слов → 11.45с —
      // граница росла линейно с длиной равнотемпового участка и ничем не
      // была ограничена (splitLongGroup вырождался в отгрызание по слову,
      // enforceTiming склеивала осколки обратно, потолок не перепроверяя).
      const words: Word[] = Array.from({ length: n }, (_, i) => w(`word${i}`, i * 0.5, i * 0.5 + 0.45));
      const cues = buildCues(words);
      for (const c of cues) expect(c.end - c.start).toBeLessThanOrEqual(MAX_DUR_SEC + 0.001);
    }
  );
