...
import { JOB_DEADLINE_MS } from "./tick";
...
export const HUNG_JOB_MESSAGE =
  "Задача не отчиталась о завершении и закрыта. Посмотри выше в чате: если ролик так и не пришёл — попробуй ещё раз через /dub.";
...
// Получасовой дедлайн живёт внутри tick, а у зависшей задачи tick как раз и не
// крутится: оборвалась цепочка — и закрыть её больше некому, кроме этого cron.
...
export async function cleanup(nowMs: number): Promise<CleanupResult> {
  ...
  for (const job of hung) {
    try {
      await sendMessage(requireEnv("TELEGRAM_DUB_BOT_TOKEN"), job.chatId, HUNG_JOB_MESSAGE);
    } catch (error) {
